...
async function fetchPrices(ticker) {
    ...
    const lookbacks = [25, 10, 5, 2]; // years
    let lastError = new Error('Invalid Finnhub candle response');

    for (const years of lookbacks) {
        try {
            const from = nowSec - Math.floor(years * 365.25 * 24 * 60 * 60);
            ...
            if (data?.s !== 'ok' || !Array.isArray(data?.c) || !Array.isArray(data?.t)) {
                lastError = new Error(data?.error || 'Invalid Finnhub candle response');
                continue;
            }
            ...
            if (prices.length === 0) {
                lastError = new Error('No price data returned');
                continue;
            }
